...
data_dir = ROOT / "Elevator" / "brf_no4fabrikoren" / "data260924"
floors = {"10A": (8, 2), "10B": (6, 1), "10C": (7, 2), "10D": (8, 2), "10E": (6, 1)}
rides = []
for i, path in enumerate(sorted(p for p in data_dir.iterdir() if p.is_file() and not p.name.startswith(".")), 1):
    match = re.search(r"(10[A-E])$", path.name)
    if not match:
        continue
    name = match.group(1)
...
payload = {"date": "2026-09-24", "label": "Uppföljningsmätning 24 sep", "status": "partial", "measuredElevators": 5, "totalElevators": 11, "missingElevators": ["6A", "4A", "4B", "4C", "4D", "4E"], "hasBarometer": True, "elevators": []}
for ride in rides:
...
(out / "2026-09-24.json").write_text(json.dumps(payload, ensure_ascii=False, separators=(",", ":")))
print(out / "2026-09-24.json")
